...
kiwi.plugin('nickserv', function(kiwi) {
    ...
    loginBtn.innerHTML = '<i aria-hidden="true" class="fa fa-sign-in"></i><span>تسجيل دخول</span>';
    loginBtn.className = 'kiwi-statebrowser-login';
    loginBtn.style.cssText = "border: 2px solid #1B9AAA; border-radius: 7px; cursor: pointer; background-color: #D4C5E2; color: #001021; padding: 5px;font-family: Noto Naskh Arabic;";
    loginBtn.addEventListener("click", loginFn);
    kiwi.addUi('browser', loginBtn);
    
    var RegBtn = document.createElement('a');
    RegBtn.className = 'kiwi-statebrowser-register';
    RegBtn.style.cssText = "border: 2px solid #1B9AAA; border-radius: 7px; cursor: pointer; background-color: #D4C5E2; color: #001021; padding: 5px;font-family: Noto Naskh Arabic;";
    RegBtn.addEventListener("click", registerFn );
    RegBtn.innerHTML = '<i aria-hidden="true" class="fa fa-lock"></i><span>تسجيل الاسم</span>';
    /*&nbsp;*/
    ...
    kiwi.on('irc.mode', function(event, network) {
        ...
        if ((event.nick == "NickServ") && (event.target == network.nick)) {
            setTimeout(function() {
                ...
                if (hasR == true) {
                        loginBtn.innerHTML = '<i aria-hidden="true" class="fa fa-sign-out"></i><span>تسجيل خروج</span>';
    ...
    kiwi.on('irc.notice', function(event) {
        ...
        if ((event.nick == 'NickServ') && (event.message.match(ValidConfirmRe))) {
                //kiwi.state.$emit('mediaviewer.hide')
                var el = document.getElementById("validate");
                el.innerHTML = "<div style=\"background-color:green;color:white;display:inline\"><b> تم التسجيل بنجاح.&nbsp;</b></div>" ;
                setTimeout(function() {
                    kiwi.state.$emit('mediaviewer.hide');
                }, 4000);
            }
        ...
        if ((event.nick == 'NickServ') && (event.message.match(RegDelayRe))) {
                var el = document.getElementById("validate");
                //el.innerHTML = event.message ;
                el.innerHTML = "<div style=\"background-color:red;color:white;display:inline\">  إنتظر 60 ثانية.&nbsp;</div>" ;
